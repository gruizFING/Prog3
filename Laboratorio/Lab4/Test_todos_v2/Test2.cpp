...
int main()
{
    printf("***********************************************************************\n");
    printf("***********************************************************************\n");
    printf("                 CASO DE PRUEBA NO OFICIAL Nro.2- Version 1.0            \n");
    printf("***********************************************************************\n");
    printf("***********************************************************************\n");
    ///CONSTRUCCIONES
    int* a = new int[1];
    ...
    for (int p=0; p<1024; p++){
        if (p<16){
            e[p]=((p*(p+p*p*p*p*p) % 661) + p+601) % 10;
            e1[p]= ((5643+ 2346 * (p + 2))%997) % 10;
            f[p]= p % 10;
            f1[p]= (p*(p+5)*(p+811) + 311) % 10;
            g[p]= (8 + p*(p %10)*e[p] ) % 10;
            h[p]= ((p + 7) % 10);
            i[p]= (8814*(p+95)*23 + 345*p+h[p]) % 10;
            j[p]= ((389 + 614*(p^2)*23 + 345*p +f[p]*e[p])%821) % 10;
            k[p]= (i[p]*p + 8*j[p] + p*(p %10)) % 10;
        }
        else if (p<32){
                f[p]= p % 10;
                f1[p]= (p*(p+54) + 998) % 10;
                g[p]= (8 + p*(p %10) ) % 10;
                h[p]= ((p + 7) % 10);
                i[p]= (612*(p+9)*23 + 345*p) % 10;
                j[p]= (666 + h[p%10] + 614*23 + 345*p ) % 10;
                k[p]= (j[p%10] + i[p%10] + 8 + p*(p %10)) % 10;
             }
             else if (p<64){
                    g[p]= (1 + 9*(p %10) ) % 10;
                    h[p]= ((p+797 + 7) % 797)%10;
                    i[p]= (61234*23+g[p] + 345*p) % 10;
                    j[p]= (666 + h[p%10] + 61234*23 + 345*p ) % 10;
                    k[p]= (j[p%10] + i[p%10]+ p*(p %10)) % 10;
                  }
                  else if (p<128){
                        h[p]= ((p*p*p) % 180)%10;
                        i[p]= (6496*(p+998)*23 + 3455*p) % 10;
                        j[p]= ((666 + h[p%10] + 61234*23 + 345*p )%601) % 10;
                        k[p]= ((j[p%10] + h[p%10] +568 + p*(p %10))%601) % 10;
                        }
                        else if (p<256){
                                i[p]= (6124*253 + 345*p) % 10;
                                j[p]= (666 + h[p%10] + 614*23 + 345*p ) % 10;
                                k[p]= ((j[p%10] + i[p%10] + 8 + p*(p %10))%991) % 10;
                             }
                             else if (p<512){
                                    j[p]= (666 + h[p%10] + 6234*23 + 345*p ) % 10;
                                    k[p]= (j[p%10] + i[p%10] + 8 + p*(p %10)) % 10;
                                  }
                                  else
                                        k[p]= (j[p%10] + i[p%10] + 8 + p*(p %10)) % 10;
